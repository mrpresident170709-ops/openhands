...
def import_from(qual_name: str):
    ...
    from its fully qualified name. For example, 'thinksoft.server.user_auth.UserAuth' would
    import the UserAuth class from the thinksoft.server.user_auth module.

    Args:
        qual_name: A fully qualified name in the format 'module.submodule.name'
                  e.g. 'thinksoft.server.user_auth.UserAuth'

    Returns:
        The imported value (class, function, or variable)

    Example:
        >>> UserAuth = import_from('thinksoft.server.user_auth.UserAuth')
        >>> auth = UserAuth()
    """
...
def get_impl(cls: type[T], impl_name: str | None) -> type[T]:
    ...
    Args:
        ...
        impl_name: Fully qualified name of the implementation class, or None to use the base class
                  e.g. 'thinksoft.server.conversation_manager.StandaloneConversationManager'
