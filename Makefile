...
# Makefile for Thinksoft project
...
start-backend:
	...
	@poetry run uvicorn thinksoft.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) --reload --reload-exclude "./workspace"
...
_run_setup:
	...
	@poetry run uvicorn thinksoft.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) &
